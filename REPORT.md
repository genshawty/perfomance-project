...
## 5. Санитайзеры: ASAN / TSAN

Результаты: `artifacts/asan_before.txt`, `asan_after.txt`, `tsan_before.txt`, `tsan_after.txt`

### До рефакторинга (broken-app-old)

| Санитайзер | Результат | Проблема |
|------------|-----------|----------|
| ASAN | **FAILED** | `get_unchecked(idx)` при `idx = len` — UB (off-by-one), abort процесса |
| TSAN | **FAILED** | Тот же UB — abort до проверки многопоточности |

Оба санитайзера детектировали нарушение precondition в `sum_even`: обращение за границу слайса через `get_unchecked`. Тест `averages_only_positive` также падал из-за некорректной логики.

### После рефакторинга (broken-app)

| Санитайзер | Результат | Тестов |
|------------|-----------|--------|
| ASAN | **OK** | 6/6 passed |
| TSAN | **OK** | 6/6 passed |

Ошибок памяти и гонок данных не обнаружено. Unsafe-код с `get_unchecked` заменён на безопасный доступ по индексу.
...
| `RUSTFLAGS="-Zsanitizer=address" cargo +nightly test --target x86_64-unknown-linux-gnu` | ASAN |
| `RUSTFLAGS="-Zsanitizer=thread" cargo +nightly test --lib --tests -Zbuild-std --target x86_64-unknown-linux-gnu` | TSAN |
